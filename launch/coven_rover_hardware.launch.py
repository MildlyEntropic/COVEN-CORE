#!/usr/bin/env python3
"""
coven_rover_hardware.launch.py - Launch a COVEN rover on real hardware

Launches all nodes needed for a physical CubeRover:
- YDLiDAR driver (publishes /scan)
- Motor driver (subscribes /cmd_vel)
- Encoder odometry (publishes /odom)
- Static TF (base_link -> laser)
- Data mule module (COVEN protocol)

Usage:
    ros2 launch coven_core coven_rover_hardware.launch.py
    ros2 launch coven_core coven_rover_hardware.launch.py rover_name:=Circe coven_name:=The_Graeae
...
def generate_launch_description():
    ...
    mule_node = Node(
        package='coven_core',
        executable='data_mule',
        name='data_mule_module',
        output='screen',
        parameters=[{
            'module_id': rover_name,
            'coven_name': coven_name,
            'robot_namespace': '',
            'use_sim_time': False,
            'record_rate_hz': 10.0,
            'max_mission_frames': 10000,
            'obstacle_threshold': 0.4,    # Slightly tighter for real world
            'dock_threshold': 0.3,
            'linear_speed': 0.2,          # Conservative speed for safety
            'angular_speed': 0.4,
            # Spawn position not used in hardware mode, but required params
            'spawn_x': 0.0,
            'spawn_y': 0.0,
            'spawn_yaw': 0.0,
        }],
    ...
    return LaunchDescription([
        ...
        # Log startup info
        LogInfo(msg=['Launching COVEN rover: ', rover_name]),
