#!/usr/bin/env python3
"""
COVEN Legacy Simulation Launch (DEPRECATED)

NOTE: This launch file uses the deprecated Python data_mule_module which has been
replaced by Rust rovers communicating via TCP to rover_bridge.py. This file is
retained for reference but will not work without the deleted data_mule_module.py.

For the current architecture, use:
- rover_bridge.py: TCP bridge for Rust rovers
- offline_slam_processor.py: Processes sensor batches from rovers
- frontier_dispatcher.py: Dispatches exploration missions
...
Usage (DEPRECATED - module no longer exists):
    ros2 launch coven_core coven_sim_legacy.launch.py witch_count:=1
