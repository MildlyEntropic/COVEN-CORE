...
class OfflineSLAMProcessor(Node):
    ...
    When a data mule reports a transfer complete, this node reads the
    recorded data and replays it (at accelerated speed) to build the map.
    """
    ...
    def __init__(self):
        ...
        # Parameters
        self.declare_parameter('data_dir', '/tmp/coven_mule_data')
        ...
        # Subscribe to data transfer notifications
        self.data_sub = self.create_subscription(
            String, '/coven/mule_data', self._data_callback, reliable_qos
        )
    ...
    def _data_callback(self, msg: String):
    ...
    def _process_mission_data(self, mission: QueuedMission):
        ...
        # Save incremental map after processing
        self.missions_processed += 1
        self._save_map(f'map_mission_{self.missions_processed:03d}')
    ...
    def _save_map(self, map_name: str):
        """Save current map to SLAM output directory."""
        if not self.slam_output_dir:
            self.get_logger().warn('[OfflineSLAM] No slam_output_dir configured, skipping map save')
            return

        import subprocess
        map_path = os.path.join(self.slam_output_dir, map_name)

        # Wait a bit for SLAM to finish processing the last scans
        self.get_logger().info(f'[OfflineSLAM] Waiting for SLAM to process, then saving to {map_path}...')
        time.sleep(2.0)

        try:
            # Use map_saver_cli from nav2_map_server
            # Explicitly specify the map topic and increase timeout
            result = subprocess.run(
                ['ros2', 'run', 'nav2_map_server', 'map_saver_cli',
                 '-f', map_path,
                 '-t', 'map',  # Explicitly specify map topic
            ...
            if result.returncode == 0:
                self.get_logger().info(f'[OfflineSLAM] Map saved: {map_path}.pgm/.yaml')
            else:
