    ...
    def _handle_coverage_task_complete(self, tc: common.TaskComplete):
        ...
        with self._coverage_lock:
            ...
            if return_reason == "low_battery":
                # Rover returned due to low battery - will recharge and be re-dispatched
                self.get_logger().info(
                    f"{tc.module_id} returned for battery recharge - will re-dispatch"
                )
                # TODO: Implement recharge delay before re-dispatch
    ...
    def _reassign_to_unexplored_sector(self, rover_id: str):
        ...
        with self._coverage_lock:
            ...
            if least_covered and min_coverage < self.coverage_target:
                # Split the least covered sector
                self.get_logger().info(
                    f"Reassigning {rover_id} to help with sector {least_covered.name}"
                )
                # For now, just dispatch to the same bounds
                # TODO: Implement sector splitting for better coordination
                if self.coverage_goal:
                    self._dispatch_coverage_task(rover_id, least_covered, self.coverage_goal)
            else:
                self.get_logger().info(
